...
public static class GetBrandsBySubCategory
{
    public sealed record Query(int SubCategoryId) : IRequest<IReadOnlyList<BrandGroupDto>>;
    ...
    public sealed class Handler : IRequestHandler<Query, IReadOnlyList<BrandGroupDto>>
    {
        ...
        {
            var key = $"brands:subcategory:{request.SubCategoryId}:v1";

            return await _cache.GetOrCreateAsync(
                key: key,
                ttl: TimeSpan.FromMinutes(60),
                factory: () => _repository.GetBrandsBySubCategoryAsync(request.SubCategoryId));
        }
    }
}
